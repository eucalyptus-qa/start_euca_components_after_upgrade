...
sub is_running {
    ...
    print "\nMACHINE $this_ip\n"; 
    print "Checking the Status of $service\n";
    ...
    print $outstr;
    if( $outstr =~ /running/ ){
        print "$service at $this_ip is currently running\n";
    }else{
	...
	for( my $i = 0; $i < $index; $i++){
		...
		if( $roll_lst[$i] =~ /CC$group/ ){
			...
			if(!is_running("$ENV{'EUCALYPTUS'}/etc/init.d/eucalyptus-cc", $my_cc_ip)) {
			    ...
			    print "\n$my_cc_ip :: $ENV{'EUCALYPTUS'}/etc/init.d/eucalyptus-cc cleanstart\n";
			    #STARTING CC
			    ...
			    if( $outstr =~ /done/ || $outstr =~ /Enabling IP/ ){
	        	        print "\nCleanstarted CC $my_cc_ip successfully !\n";
			    }else{
				if(!is_running("$ENV{'EUCALYPTUS'}/etc/init.d/eucalyptus-cc", $my_cc_ip)) {
				    print "\n[TEST_REPORT]\tFAILED to Cleanstart CC $my_cc_ip !!\n";
				    $is_error = 1;
				}
				#exit(1);
			    };
...
sub enable_vmbroker{
	...
	for( my $i = 0; $i < $index; $i++){
		if( does_It_Have($roll_lst[$i], "CC") ){
			...
			if( $is_vmbroker == 1 && $source_lst[$i] eq "BZR" ){
				my $cc_ip = $ip_lst[$i];
...
sub initialize_master_clc{

	my $master_ip = $ENV{'QA_MASTER_CLC_IP'};

	print "\n\n----------------------- Initializing CLC $master_ip -----------------------\n";

	print "\nMonitoring EUCA components\n";
	
	print("ssh -o StrictHostKeyChecking=no root\@$master_ip \"ps axwww | grep euca\"\n");
	my $outstr = `ssh -o StrictHostKeyChecking=no root\@$master_ip \"ps axwww | grep euca\"`;

	print $outstr;
	print "\n\n";
	

	print "\n$master_ip :: $ENV{'EUCALYPTUS'}/usr/sbin/euca_conf --initialize\n";

	###Initializing Master CLC
	print("ssh -o StrictHostKeyChecking=no root\@$master_ip \"$ENV{'EUCALYPTUS'}/usr/sbin/euca_conf --initialize\"\n");
	$outstr = `ssh -o StrictHostKeyChecking=no root\@$master_ip \"$ENV{'EUCALYPTUS'}/usr/sbin/euca_conf --initialize\"`;
...
sub start_cloud_components{
	...
	for( my $j = 0; $j < $index; $j++ ){
		...
		if( does_It_Have( $this_roll, "CLC") || does_It_Have( $this_roll, "SC") || does_It_Have( $this_roll, "WS") || does_It_Have( $this_roll, "VB")  ){
			...
			$outstr = `ssh -o StrictHostKeyChecking=no root\@$this_ip \"killall dnsmasq\"`;
